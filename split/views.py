...
from .serializer import UserProfileSerializer, GroupSerializer, ExpenseSerializer
...
class ShowUserDetailsApiView(APIView):
    ...
    def get(self, request) -> Response:
        ...
        try:
            user = UserProfile.objects.get(email=user_email)
            f_debts = Debt.objects.filter(from_user=user)
            t_debts = Debt.objects.filter(to_user=user)
            debt_data = dict()
            debit = 0
            credit = 0
            for i in f_debts:
...
class CreateExpenseApiView(APIView):
    ...
    def post(self, request) -> Response:
        ...
        all_users = UserProfile.objects.filter(email__in=all_users)
        paid_by = request.data.get('paid_by')
        paid_by_user = UserProfile.objects.filter(email=paid_by).first()
        amount = request.data.get('amount')
        group_name = request.data.get('group_name', None)
        expense_name = request.data.get('name')
        if Expense.objects.filter(name=expense_name).count() > 0:
            return Response({
        ...
        for user in all_users:
            if user != paid_by_user:
                debt = Debt.objects.create(**{"from_user": paid_by_user,
                                                     "to_user": user,
            ...
            expense_user = ExpenseUser.objects.create(**expense_user_dict)
        ...
        expense = Expense.objects.create(**expense)
    ...
    def post(self, request) -> Response:
        ...
        try:
            if group_name is None:
                ...
                expense = Expense.objects.get(name=expense_name)
                if expense.expense_group != Group.objects.get(group_name=group_name):
                    return Response({
